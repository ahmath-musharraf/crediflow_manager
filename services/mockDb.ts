...
const getSafeUrl = (url: string) => {
  ...
  // Remove channel_binding which is incompatible with browser WebSockets
  return cleanUrl
...
class MockDbService {
  ...
  async init() {
    ...
    // Always load local data first so UI has something to show
  ...
  getProducts(shopId?: string) { return shopId ? this.products.filter(p => p.shopId === shopId) : this.products; }
  getCustomers() { return this.customers; }
  getTransactions(shopId?: string) { return shopId ? this.transactions.filter(t => t.shopId === shopId) : this.transactions; }
  getActivities(shopId?: string) { 
    let filtered = this.activities;
    if (shopId) filtered = this.activities.filter(a => a.shopId === shopId);
    return filtered.slice().sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()); 
  }
  getActivitiesByCustomer(customerId: string) {
    return this.activities.filter(a => a.customerId === customerId).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }
  getTransactionsByCustomer(customerId: string) { return this.transactions.filter(t => t.customerId === customerId); }
  getPaymentsByCustomer(customerId: string) { return this.payments.filter(p => p.customerId === customerId); }
  getExpensesByCustomer(customerId: string) { return this.expenses.filter(e => e.customerId === customerId); }

  // --- WRITES ---
  private async log(action: string, description: string, user?: User, shopId?: string, customerId?: string) {
    const logEntry: ActivityLog = {
      id: `log-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
      shopId, customerId, date: new Date().toISOString(), action, description, performedBy: user ? user.username : 'Unknown', shopName: user?.shopName
    };
  ...
  async updateProduct(productId: string, updates: Partial<Product>, user?: User, reason?: string) {
    ...
    if (index > -1) {
      ...
      this.saveToLocalStorage();
      this.log('UPDATE_PRODUCT', `Updated ${oldData.name}${reason ? ` | ${reason}` : ''}`, user, oldData.shopId);
      if (sql) {
  ...
  async addProducts(products: Product[], user?: User) {
    ...
    this.saveToLocalStorage();
    this.log('IMPORT_CSV', `Imported ${products.length} products`, user, products[0]?.shopId);
    if (sql) {
  ...
  async transferProduct(productName: string, fromShopId: string, toShopId: string, quantity: number, user?: User) {
    ...
    if (destIndex > -1) {
      ...
      const newProduct = { ...this.products[sourceIndex], id: `PROD-${Date.now()}-${Math.random()}`, shopId: toShopId, stock: quantity };
      this.products.push(newProduct);
      if (sql) await sql`INSERT INTO products (id, shop_id, name, category, price, wholesale_price, stock, description) VALUES (${newProduct.id}, ${newProduct.shopId}, ${newProduct.name}, ${newProduct.category}, ${newProduct.price}, ${newProduct.wholesalePrice}, ${newProduct.stock}, ${newProduct.description || ''})`;
    }
  ...
  async addCustomer(customer: Customer, user?: User) {
    ...
    this.saveToLocalStorage();
    this.log('ADD_CUSTOMER', `New customer: ${customer.name}`, user, undefined, customer.id);
    if (sql) await sql`INSERT INTO customers (id, name, phone, type, shop_name, location, whatsapp, credit_limit, total_debt) VALUES (${customer.id}, ${customer.name}, ${customer.phone}, ${customer.type}, ${customer.shopName || ''}, ${customer.location || ''}, ${customer.whatsapp || ''}, ${customer.creditLimit || 0}, ${customer.totalDebt})`;
  }
  ...
  async updateCustomer(id: string, updates: Partial<Customer>, user?: User) {
    ...
    if (index > -1) {
      ...
      this.saveToLocalStorage();
      this.log('UPDATE_CUSTOMER', `Updated ${this.customers[index].name}`, user, undefined, id);
      if (sql) {
  ...
  async createTransaction(transaction: Transaction, user?: User) {
    ...
    this.saveToLocalStorage();
    this.log('SALE', `Inv #${transaction.id.slice(-4)} for ${transaction.customerName} (Rs. ${transaction.totalAmount})`, user, transaction.shopId, transaction.customerId);
  ...
  async recordPayment(customerId: string, amount: number, user?: User, shopId?: string) {
    ...
    if (custIndex > -1) {
      ...
      const payment: PaymentRecord = { id: Math.random().toString(36).substr(2, 9), shopId: shopId || 'unknown', customerId, amount, date: new Date().toISOString() };
      this.payments.push(payment);
      this.saveToLocalStorage();
      this.log('PAYMENT', `Payment Rs. ${amount} from ${this.customers[custIndex].name}`, user, shopId, customerId);
      if (sql) {
        await sql`UPDATE customers SET total_debt = ${this.customers[custIndex].totalDebt} WHERE id = ${customerId}`;
        await sql`INSERT INTO payments (id, shop_id, customer_id, amount, date) VALUES (${payment.id}, ${payment.shopId}, ${payment.customerId}, ${payment.amount}, ${payment.date})`;
      }
    }
  }
  ...
  async recordExpense(customerId: string, description: string, amount: number, date: string, user?: User, shopId?: string) {
    ...
    if (custIndex > -1) {
      this.customers[custIndex].totalDebt += amount;
      const expense: ExpenseRecord = { id: `exp-${Date.now()}`, shopId: shopId || 'unknown', customerId, description, amount, date };
      this.expenses.push(expense);
      this.saveToLocalStorage();
      this.log('EXPENSE', `Expense: ${description} (Rs. ${amount})`, user, shopId, customerId);
      if (sql) {
        await sql`UPDATE customers SET total_debt = ${this.customers[custIndex].totalDebt} WHERE id = ${customerId}`;
        await sql`INSERT INTO expenses (id, shop_id, customer_id, description, amount, date) VALUES (${expense.id}, ${expense.shopId}, ${expense.customerId}, ${expense.description}, ${expense.amount}, ${expense.date})`;
      }
    }
  }
}
...
export const authenticate = (username: string, password: string): User | null => {
  if (username === 'admin' && password === 'admin') return { id: 'u1', username: 'Super Admin', role: Role.SUPER_ADMIN };
  return null;
};
...
export const authenticateShop = (shopId: string, password: string): User | null => {
  if (password === '1234') {
    ...
    if (shop) return { id: `shop-admin-${shop.id}`, username: `${shop.name} Manager`, role: Role.SHOP_ADMIN, shopName: shop.name };
  }
  return null;
};
